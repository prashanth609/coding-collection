public class SolidDemo {
  ...
  static class Invoice {
    private final int id; private final double amount;
    Invoice(int id, double amount){ this.id=id; this.amount=amount; }
    int getId(){ return id; } double getAmount(){ return amount; }
  }
  static class InvoicePrinter { void print(Invoice inv){
    System.out.println("Invoice #" + inv.getId() + " amount=" + inv.getAmount());
  }}
  static class InvoiceSaver { void save(Invoice inv){
    System.out.println("Saved invoice " + inv.getId());
  }}

  // ===== O — Open/Closed =====
  interface Shape { double area(); }
  static class Circle implements Shape { private final double r;
    Circle(double r){ this.r=r; } public double area(){ return Math.PI*r*r; } }
  static class Rectangle implements Shape { private final double w,h;
    Rectangle(double w,double h){ this.w=w; this.h=h; } public double area(){ return w*h; } }
  ...
  static class Sparrow implements Bird, Flyable {
    public void eat(){ System.out.println("Sparrow pecks."); }
  ...
  static void letItFly(Flyable f){ f.fly(); } // only things that can actually fly
  ...
  static class MultiFunctionPrinter implements Printer, Scanner {
    public void print(String doc){ System.out.println("MFP print: " + doc); }
  ...
  static class NotificationService {
    ...
    NotificationService(MessageSender sender){ this.sender = sender; } // depend on abstraction
    void notify(String user, String message){ sender.send(user, message); }
  }
  ...
  public static void main(String[] args) {
    ...
    // ISP
    Printer p = new SimplePrinter(); p.print("Report");
    MultiFunctionPrinter mfp = new MultiFunctionPrinter(); mfp.scan();
